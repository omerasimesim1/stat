...
logs ( client );
let pub = "966816594649026590"; ////// kategori id
let kayıt = "966816576068263966"
let terapi = ""
let sorun = "966816590106591292"
let vk = "966816598834937872"
let dc = "966816598834937872"
let game = ""
let priv = "966816602442051614"
let alone = "966816801625366598"
...
client.on ( "voiceChannelJoin" , ( member , channel ) => {
    ...
    if (!member.roles.cache.has ("800779218430459948", "800779218370691117", "800779218370691116", "800779218354307111",)) return
    const json = {
        "channel" : channel.id ,
...
client.on ( "message" , async message => {
    ...
    if(!["800779218430459948", "800779218370691117", "800779218370691116", "800779218354307111",].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.react('❌')
    var totall = (
                     await db.fetch (
...
client.on ( "message" , async msg => {
    if ( msg.content.startsWith ( "qua!me" ) || msg.content.startsWith ( ".me" ) ) {
        ...
        if(!["800779218430459948", "800779218370691117", "800779218370691116", "800779218354307111",].some(role => msg.member.roles.cache.get(role)) && !msg.member.hasPermission('ADMINISTRATOR')) return msg.react('❌')
        let st = member.voice
...
───────────────
\`> \` **Sesli Kanal Sıralaması**:(\`Toplam ${ sayi } kanalda durmuş\`)
${ isimler.join ( "\n" ) }
───────────────
...
client.on ( "message" , async msj => {
    ...
    if(!["800779218430459948", "800779218370691117", "800779218370691116", "800779218354307111",].some(role => msj.member.roles.cache.get(role)) && !msj.member.hasPermission('ADMINISTRATOR')) return msj.react('❌')
    if ( ! msj.content.startsWith ( "qua!top" ) ) {
        return;
    }
    let data = await db
        .all ()
    ...
    const toplam = new Discord.MessageEmbed ()
        ...
        .setDescription ( `• Sunucunun Ses Bilgisi

Messages | Top 5 - Members
${ aruuy.join ( "\n" ) }

Messages | Top 5 - Channel
${ arvy.join ( "\n" ) }
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Q U A D R I S E"}, status: "online" }); //dnd rahatsı etmeyın, online çevirimiçi, idle boşta 
});
